...
class TestFusionBasics:
    ...
    def test_score_matches_the_formula(self) -> None:
        fused = reciprocal_rank_fusion(
            {"dense": ["a"], "sparse": ["a"]}, key_fn=ident, k=60
        )
...
class TestProvenance:
    def test_per_retriever_ranks_are_retained(self) -> None:
        ...
        fused = reciprocal_rank_fusion(
            {"dense": ["x", "a"], "sparse": ["a", "y"]}, key_fn=ident
        )
...
class TestWeighting:
    def test_sparse_weight_promotes_lexical_hits(self) -> None:
        heavy_sparse = weighted_rrf(
            ["dense_hit"], ["sparse_hit"], key_fn=ident, sparse_weight=5.0
        )
        assert heavy_sparse[0].key == "sparse_hit"

    def test_dense_weight_promotes_semantic_hits(self) -> None:
        heavy_dense = weighted_rrf(
            ["dense_hit"], ["sparse_hit"], key_fn=ident, dense_weight=5.0
        )
...
class TestDeterminism:
    def test_ordering_is_stable_across_runs(self) -> None:
        ...
        runs = [
            [r.key for r in reciprocal_rank_fusion(lists, key_fn=ident)]
            for _ in range(5)
        ]
...
class TestRealisticShape:
    def test_key_fn_deduplicates_structured_items(self) -> None:
        ...
        fused = reciprocal_rank_fusion(
            {"dense": dense, "sparse": sparse}, key_fn=lambda d: d["id"]
        )
        assert len(fused) == 3
    ...
    def test_top_n_truncates(self) -> None:
        fused = reciprocal_rank_fusion(
            {"dense": list("abcdefghij")}, key_fn=ident, top_n=3
        )
        assert len(fused) == 3
