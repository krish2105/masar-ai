...
_SIGNALS: dict[Intent, list[str]] = {
    Intent.FARE_COST: [
        r"\bfare\b", r"\bcost\b", r"\bprice\b", r"\bcheap(er|est)?\b", r"\bexpensive\b",
        r"\bnol\b", r"\bsalik\b", r"\btoll\b", r"\bzone[s]?\b", r"\bAED\b", r"\bdirham\b",
        r"\bhow much\b", r"\bmonthly (cost|pass)\b", r"\bbudget\b",
        r"أجرة", r"سعر", r"تكلفة", r"أرخص", r"كم يكلف", r"نول", r"سالك", r"درهم",
    ],
    Intent.NETWORK_ANALYTICS: [
        r"\bridership\b", r"\btrend\b", r"\bbusiest\b", r"\bmost used\b", r"\bpassengers?\b",
        r"\bhow many (trips|passengers|people)\b", r"\bgrowth\b", r"\bcompare\b",
        r"\bmodal split\b", r"\butili[sz]ation\b", r"\bstatistics\b", r"\btop \d+\b",
        r"\bover time\b", r"\bper month\b", r"\bannual\b",
        r"ازدحام", r"الأكثر", r"عدد الركاب", r"إحصائيات", r"اتجاه",
    ],
    Intent.GEOSPATIAL: [
        r"\bnearest\b", r"\bclosest\b", r"\bnear\b", r"\bwithin \d+\s*(km|m|metre)",
        r"\bhow far\b", r"\bdistance\b", r"\bwalking\b", r"\baround\b", r"\bcatchment\b",
        r"أقرب", r"قريب", r"المسافة", r"كم تبعد",
    ],
    Intent.JOURNEY_PLANNING: [
        r"\bhow (do|can) i get\b", r"\bfrom\b.*\bto\b", r"\bconnect(s|ion)?\b",
        r"\broute[s]? (to|from|between)\b", r"\bwhich (bus|metro|tram|line)\b",
        r"\binterchange\b", r"\btransfer\b", r"\bserve[s]?\b", r"\bgo(es)? to\b",
        r"كيف أصل", r"من .* إلى", r"أي خط", r"يربط",
    ],
    Intent.SERVICE_INFO: [
        r"\bhow do i (get|apply|replace|renew)\b", r"\bwhat documents?\b",
        r"\brequire(d|ment)\b", r"\bopening hours\b", r"\beligib", r"\bcard type\b",
        r"\bwhat is\b.*\b(nol|salik|zone)\b", r"\bexplain\b", r"\bdifference between\b",
        r"ما هي", r"كيف أحصل", r"المستندات", r"ساعات العمل",
    ],
}
...
class IntentAgent:
    ...
    def classify_keywords(self, text: str) -> IntentResult:
        ...
        if not hits:
            return IntentResult(
                intent=Intent.MULTI_HOP, confidence=0.0, method="keyword",
                reasoning="no intent signals matched", signals=signals,
            )
    ...
    async def run(self, text: str) -> IntentResult:
        ...
        except Exception as exc:  # noqa: BLE001 — keywords are the fallback
