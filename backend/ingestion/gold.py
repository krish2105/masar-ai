...
class GoldBuilder:
    ...
    def _provenance(frame: pl.DataFrame, dataset_id: str) -> pl.DataFrame:
        ...
        return frame.with_columns(
            ...
            (pl.col("_source_url") if "_source_url" in frame.columns else pl.lit("")).alias(
                "source_url"
            ),
            (pl.col("_captured_at") if "_captured_at" in frame.columns else pl.lit("")).alias(
                "captured_at"
            ),
            (
                pl.col("_source_tier") if "_source_tier" in frame.columns else pl.lit("archive")
            ).alias("source_tier"),
    ...
    def build_dim_station(self) -> pl.DataFrame:
        ...
        if (frame := self.silver("transport_stations")) is not None:
            ...
            selected = frame.select(
                ...
                .cast(pl.Utf8)
                .alias("station_id"),
                pl.col(name_en).cast(pl.Utf8).alias("station_name_en")
                if name_en
                else pl.lit(None, pl.Utf8).alias("station_name_en"),
                pl.col(name_ar).cast(pl.Utf8).alias("station_name_ar")
                if name_ar
        ...
        if (frame := self.silver("metro_stations")) is not None:
            ...
            selected = frame.select(
                ...
                pl.col("latitude").cast(pl.Float64).alias("latitude"),
                pl.col("longitude").cast(pl.Float64).alias("longitude"),
                (pl.col(opened).cast(pl.Utf8) if opened else pl.lit(None, pl.Utf8)).alias(
                    "opened_on"
                ),
            )
        ...
        if (frame := self.silver("marine_stations")) is not None:
            selected = frame.select(
                ...
                if "route_name" in frame.columns
                else pl.lit(None, pl.Utf8).alias("line_name"),
    ...
    def build_dim_stop(self) -> pl.DataFrame:
        ...
        selected = frame.select(
            ...
            if "street_name" in frame.columns
            else pl.lit(None, pl.Utf8).alias("street_name"),
            pl.col("latitude").cast(pl.Float64).alias("latitude")
            if "latitude" in frame.columns
            else pl.lit(None, pl.Float64).alias("latitude"),
            pl.col("longitude").cast(pl.Float64).alias("longitude")
            ...
            if "bus_stop_type" in frame.columns
            else pl.lit(None, pl.Utf8).alias("stop_type"),
        )
    ...
    def build_dim_route(self) -> pl.DataFrame:
        ...
        if (frame := self.silver("routes_stops")) is not None:
            aggregated = frame.group_by(["route_name", "transport_mode"]).agg(
                ...
                if "depot_name" in frame.columns
                else pl.lit(None).first().alias("operator"),
                pl.col("route_length").drop_nulls().first().alias("route_length_km")
                if "route_length" in frame.columns
                else pl.lit(None).first().alias("route_length_km"),
                pl.col("stop_id").n_unique().alias("stop_count"),
                pl.col("_source_url").first().alias("_source_url")
                if "_source_url" in frame.columns
                else pl.lit("").first().alias("_source_url"),
                pl.col("_captured_at").first().alias("_captured_at")
                if "_captured_at" in frame.columns
                else pl.lit("").first().alias("_captured_at"),
            )
    ...
    def build_bridge_route_stop(self) -> pl.DataFrame:
        ...
        selected = frame.select(
            ...
            pl.col("stop_id").cast(pl.Utf8).alias("stop_id"),
            pl.col("stop_name").cast(pl.Utf8).alias("stop_name_en"),
            pl.col("stop_order_number")
            .cast(pl.Float64)
            .cast(pl.Int32, strict=False)
            .alias("stop_order")
            if "stop_order_number" in frame.columns
            else pl.lit(None, pl.Int32).alias("stop_order"),
            pl.col("route_direction").cast(pl.Utf8).alias("direction")
            if "route_direction" in frame.columns
            else pl.lit(None, pl.Utf8).alias("direction"),
            pl.col("latitude").cast(pl.Float64).alias("latitude")
            if "latitude" in frame.columns
            else pl.lit(None, pl.Float64).alias("latitude"),
            pl.col("longitude").cast(pl.Float64).alias("longitude")
    ...
    def build_fact_ridership(self) -> pl.DataFrame:
        ...
        if anomalous:
            log.warning(
                ...
                rows=anomalous,
                detail=(
                    "periods whose per-entity median differs from the mode baseline "
                    f"by more than {SCALE_ANOMALY_FACTOR}x — probably a different unit "
                    "or reporting period"
                ),
            )
    ...
    def build_fact_modal_split(self) -> pl.DataFrame:
        ...
        before = facts.height
        facts = facts.filter(
            pl.col("transport_type")
            .str.strip_chars()
            .str.to_lowercase()
            .is_in(list(VALID_TRANSPORT_TYPES))
        )
    ...
    def build_dim_taxi_stand(self) -> pl.DataFrame:
        ...
        selected = frame.select(
            (pl.col(identifier).cast(pl.Utf8) if identifier else pl.lit(None, pl.Utf8)).alias(
                "stand_id"
            ),
        ...
        stands = (
            stands.with_row_index("_row")
            .with_columns(
                pl.coalesce(
                    [pl.col("stand_id"), pl.lit("stand_") + pl.col("_row").cast(pl.Utf8)]
                ).alias("stand_id")
            )
            .drop("_row")
        )
    ...
    def build_dim_taxi_driver_profile(self) -> pl.DataFrame:
        ...
        selected = frame.select(
            ...
            else pl.lit(None, pl.Utf8).alias("report_date"),
            pl.col("operator_type").cast(pl.Utf8).alias("operator_type")
            if "operator_type" in frame.columns
            else pl.lit(None, pl.Utf8).alias("operator_type"),
            (pl.col(operator).cast(pl.Utf8) if operator else pl.lit(None, pl.Utf8)).alias(
                "operator_name"
            ),
            (pl.col(count).cast(pl.Float64) if count else pl.lit(None, pl.Float64)).alias(
                "driver_count"
            ),
        )
    ...
    def build_dim_salik_gate(self) -> pl.DataFrame:
        ...
        name_en = self._pick(
            frame, "gate_name_en", "gate_name", "name_en", "toll_gate_name", "name"
        )
        name_ar = self._pick(frame, "gate_name_ar", "name_ar")
        selected = frame.select(
            (pl.col(name_en).cast(pl.Utf8) if name_en else pl.lit(None, pl.Utf8)).alias(
                "gate_name_en"
            ),
            (pl.col(name_ar).cast(pl.Utf8) if name_ar else pl.lit(None, pl.Utf8)).alias(
                "gate_name_ar"
            ),
            pl.col("latitude").cast(pl.Float64).alias("latitude")
            if "latitude" in frame.columns
        ...
        return self._write("dim_salik_gate", gates.with_columns(pl.col("gate_id").cast(pl.Utf8)))
    ...
    return _empty(
        {
            ...
            "station_name_ar": pl.Utf8,
            "station_name_ar_norm": pl.Utf8,
            "mode": pl.Utf8,
            "line_name": pl.Utf8,
            "zone_id": pl.Int32,
            "latitude": pl.Float64,
...
def _empty_stop() -> pl.DataFrame:
    return _empty(
        {
            "stop_id": pl.Utf8,
            "stop_name_en": pl.Utf8,
            "stop_name_ar": pl.Utf8,
            "mode": pl.Utf8,
            "street_name": pl.Utf8,
            "latitude": pl.Float64,
            "longitude": pl.Float64,
...
def _empty_route() -> pl.DataFrame:
    return _empty(
        {
            "route_key": pl.Utf8,
            "route_number": pl.Utf8,
            "route_name_en": pl.Utf8,
            "route_name_ar": pl.Utf8,
            "mode": pl.Utf8,
            "route_type": pl.Utf8,
            "origin_en": pl.Utf8,
            "destination_en": pl.Utf8,
            "operator": pl.Utf8,
            "route_length_km": pl.Float64,
            "stop_count": pl.Int32,
            "source_dataset": pl.Utf8,
            "source_url": pl.Utf8,
            "captured_at": pl.Utf8,
...
def _empty_taxi_driver() -> pl.DataFrame:
    return _empty(
        {
            "report_date": pl.Utf8,
            "operator_type": pl.Utf8,
            "operator_name": pl.Utf8,
            "driver_count": pl.Float64,
            "source_dataset": pl.Utf8,
            "source_url": pl.Utf8,
            "captured_at": pl.Utf8,
            "source_tier": pl.Utf8,
...
def _empty_salik_gate() -> pl.DataFrame:
    return _empty(
        {
            "gate_id": pl.Utf8,
            "gate_name_en": pl.Utf8,
            "gate_name_ar": pl.Utf8,
            "latitude": pl.Float64,
            "longitude": pl.Float64,
            "source_dataset": pl.Utf8,
            "source_url": pl.Utf8,
    ...
    return _empty(
        {
            ...
            "year": pl.Int64,
            "month": pl.Int64,
            "month_name_en": pl.Utf8,
            "month_name_ar": pl.Utf8,
            "quarter": pl.Int64,
            "year_month": pl.Utf8,
        }
    )
